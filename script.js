...
let elItem = document.querySelector('.js-item');
...
async function davlatlar() {
    ...
    elSelect.addEventListener('change', async(event) => {
        async function filterRegion() {
            const response = await fetch(`https://restcountries.com/v3.1/region/${event.target.value}`);
            const filteredData = await response.json();
            renderDavlatlar(filteredData, elList);
        }
...
function renderDavlatlar(array, element) {
    element.innerHTML = '';
